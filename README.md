...
Note the install commands are allowed by any user on any interface when required, so it is recommended to have public web access disabled during install.  You can just delete or disable access to the install.php entry point if you will only be using CLI for install/upgrade.
