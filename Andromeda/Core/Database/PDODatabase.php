...
class PDODatabase
{
     ...
     * @throws DatabaseConfigException if the given file does not return an array
     * @return array<mixed>
     */
    public static function LoadConfig(?string $path = null) : array
    {
        ...
        if ($path !== null)
        {
            ...
            else throw new DatabaseConfigException('not array'); 
        }
    ...
    public static function Install(SafeParams $params) : ?string
    {
        ...
        if ($params->HasParam('outfile')) // store it
        {
            ...
            catch (PDODatabaseConnectException $e) 
            {
                unlink($tmpnam); 
     ...
     * @throws DatabaseConfigException if the driver in the config is invalid
     * @throws PDODatabaseConnectException if the connection fails
     */
    public function __construct(array $config, bool $verbose = false, ?DBStats $init_stats = null)
    {
        ...
        if (!array_key_exists('DRIVER',$config))
            throw new DatabaseConfigException('missing DRIVER');
        if (!array_key_exists('CONNECT',$config))
            throw new DatabaseConfigException('missing CONNECT');
        
        $driver = $config['DRIVER'];
        if (!array_key_exists($driver, self::DRIVERS))
            throw new DatabaseConfigException("driver $driver");
        ...
        catch (PDOException $e){ throw new PDODatabaseConnectException($verbose ? $e : null); }
     ...
     * @throws ImportFileMissingException if path does not exist
     */
    public function importFile(string $path) : self
    {
        if (($data = file($path)) === false) throw new ImportFileMissingException($path);
        ...
        $queries = array_filter(array_map(function(string $query){ return trim($query); }, $queries));
